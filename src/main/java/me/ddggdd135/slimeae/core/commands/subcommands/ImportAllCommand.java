...
public class ImportAllCommand extends SubCommand {
    @Override
    @Nonnull
    ...
    public boolean onCommand(
        ...
        if (!exportDir.exists() || !exportDir.isDirectory()) {
            sender.sendMessage(CMIChatColor.translate("&c导出目录不存在: " + exportDir.getAbsolutePath()));
            return false;
        }
        sender.sendMessage(CMIChatColor.translate("&e正在导入所有元件数据..."));
        try {
            ...
            int count = eim.importAll(exportDir);
            sender.sendMessage(CMIChatColor.translate("&a导入成功: 共计 " + count + " 个物品"));
        } catch (Exception e) {
